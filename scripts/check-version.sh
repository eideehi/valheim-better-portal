...
project_file="BetterPortal/BetterPortal.csproj"
plugin_file="BetterPortal/BetterPortal.cs"
manifest_file="distributor/thunderstore/manifest.json"
assembly_info="BetterPortal/Properties/AssemblyInfo.cs"
...
check_thunderstore_links() {
    ...
    while IFS= read -r entry; do
        ...
        {
            (grep -Eo 'github\.com/eideehi/valheim-better-portal/blob/[^)[:space:]]+' "$repo_root/$thunderstore_readme" || true) \
                | sed -E 's|^.*blob/||'
            (grep -Eo 'raw\.githubusercontent\.com/eideehi/valheim-better-portal/[^)[:space:]]+' "$repo_root/$thunderstore_readme" || true) \
                | sed -E 's|^.*valheim-better-portal/||'
        } | sed -E 's/[?#].*$//' | sort -u
    )
...
if grep -Eq 'github\.com/eideehi/valheim-better-portal/blob/(main|master)/|raw\.githubusercontent\.com/eideehi/valheim-better-portal/(main|master)/' "$repo_root/$thunderstore_readme"; then
